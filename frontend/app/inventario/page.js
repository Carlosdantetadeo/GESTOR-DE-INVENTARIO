    ...
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div className="glass-card" style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'center', padding: '12px 16px' }}>
        <div style={{ flex: 1, minWidth: '180px', position: 'relative' }}>
          <Search size={14} style={{ position: 'absolute', left: '11px', top: '50%', transform: 'translateY(-50%)', color: 'hsl(var(--text-muted))' }} />
          <input
            type="text"
        ...
        <select value={categoria} onChange={e => setCategoria(e.target.value)} className="input-field" style={{ width: '170px' }}>
          <option value="all">Todas las categorías</option>
          {categorias.map(c => <option key={c} value={c}>{c}</option>)}
        </select>
        <select value={tiendaFiltro} onChange={e => setTiendaFiltro(e.target.value)} className="input-field" style={{ width: '150px' }}>
      ...
      <div className="glass-card" style={{ padding: 0, overflow: 'hidden' }}>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={7 + pivotTiendas.length} style={{ textAlign: 'center', color: 'hsl(var(--text-muted))', padding: '48px' }}>Cargando…</td></tr>
              ) : sorted.length === 0 ? (
                <tr><td colSpan={7 + pivotTiendas.length} style={{ textAlign: 'center', color: 'hsl(var(--text-muted))', padding: '48px' }}>Sin productos con los filtros aplicados.</td></tr>
              ) : sorted.map(prod => {
                const total = getTotalStock(prod)
                  ...
                  <tr key={prod.id}
                    ...
                    <td style={{ padding: '11px 16px' }}>
                      ...
                      <div style={{ display: 'flex', gap: '8px', marginTop: '3px', flexWrap: 'wrap' }}>
                        {prod.referencia && (
                          <span style={{ fontSize: '0.7rem', color: 'hsl(var(--text-muted))', fontFamily: 'monospace', background: 'hsl(var(--bg-base))', padding: '1px 6px', borderRadius: '4px', border: '1px solid hsl(var(--border))' }}>
                            {prod.referencia}
                          </span>
                        )}
                        <span style={{ fontSize: '0.7rem', color: 'hsl(var(--text-muted))' }}>{prod.unidad}</span>
                      </div>
                    </td>
                    {/* Categoría */}
                    <td style={{ padding: '11px 16px' }}>
                      <span style={{ fontSize: '0.74rem', background: 'hsl(var(--bg-base))', padding: '3px 8px', borderRadius: '4px', border: '1px solid hsl(var(--border))' }}>
                        {prod.categoria}
                      </span>
                    </td>
                    {/* Stock por sede */}
                    ...
                    {/* Mínimo (solo lectura) */}
                    <td style={{ padding: '11px 16px', textAlign: 'right', color: 'hsl(var(--text-secondary))' }}>{prod.stockMinimo}</td>
                    {/* Costo */}
                    <td style={{ padding: '11px 16px', textAlign: 'right', color: 'hsl(var(--text-secondary))' }}>S/ {prod.costo.toFixed(2)}</td>
                    {/* Precio */}
                    <td style={{ padding: '11px 16px', textAlign: 'right', color: 'hsl(var(--text-secondary))' }}>S/ {prod.sugerido.toFixed(2)}</td>
                    {/* Valor */}
        ...
        {!loading && sorted.length > 0 && (
          <div style={{ padding: '10px 16px', borderTop: '1px solid hsl(var(--border))', fontSize: '0.78rem', color: 'hsl(var(--text-muted))', display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: '8px' }}>
            <span>{sorted.length} producto{sorted.length !== 1 ? 's' : ''}</span>
            <span>Valor total: <strong style={{ color: 'hsl(var(--text-primary))' }}>S/ {fmt(valorTotal)}</strong></span>
          </div>
        )}
      </div>
    </div>
