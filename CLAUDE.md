...
| `migrations/022_modelos_api_key.sql` | Adds `modelos_nlu.api_key_enc TEXT` — per-model API key, **encrypted at rest** (AES-GCM-256, master key `MODELOS_ENC_KEY`, format `v1.<iv>.<ct>`) by the app (`lib/superadmin/cryptoModelo.js`). The DB never sees plaintext and `getModelosNlu` never returns the blob (only a `tiene_api_key` flag). The bot decrypts at runtime (`descifrarApiKey()`), falling back to the provider secret if a model has none. |
| `migrations/023_modelos_base_url_openai_compat.sql` | Adds `modelos_nlu.base_url TEXT` and extends the `proveedor` CHECK to include **`openai-compat`** — a generic OpenAI-compatible provider for any endpoint (e.g. **Huawei Cloud MaaS / ModelArts**). The bot reuses the OpenAI-compatible branch pointing at `base_url` (appends `/chat/completions` if absent) and uses the per-model key only (no provider-secret fallback for third-party endpoints). |
...
- **NLU**: Per-empresa model selected in `empresas.nlu_model` (an `id` into the `modelos_nlu` catalog). As of sprint 021 the model list is **dynamic, DB-driven**: the bot calls `resolverModelo(nlu_model)` at runtime (migration 021), which reads the row from `modelos_nlu` and dispatches `callNLU` by `proveedor`. If the lookup fails (table missing, unknown id, etc.) it **falls back to `groq-llama`**. The seeded catalog mirrors the previous hardcode:
  ...
  - new models (e.g. OpenRouter, or any OpenAI-compatible endpoint like Huawei MaaS via `openai-compat`) are added from `/superadmin/modelos`, no code change.
  - **Providers:** `groq`, `openrouter` and `openai-compat` share the OpenAI-compatible branch in `callNLU`; `anthropic` uses the Messages API. URL resolution: `groq`/`openrouter` are hardcoded, `openai-compat` uses the model's `base_url` (migration 023, e.g. Huawei MaaS) and appends `/chat/completions` if absent. Key resolution: per-model `api_key_enc` (migration 022) first; `groq`/`openrouter` fall back to `GROQ_API_KEY`/`OPENROUTER_API_KEY` secrets, but `openai-compat` uses **only** the per-model key (never sends a provider secret to a third-party endpoint). Decryption needs `MODELOS_ENC_KEY` on the bot.
